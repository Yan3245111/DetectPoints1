...
"""
def detect_spheres_from_dicom(dicom_path: str, num_spheres: int = 3, r_range: float = 2.5, rlen: int = 5,
                              rmax: int = 4):
    import taichi as ti
    import numpy as np
    import pydicom
    ...
    ImagePositionPatient = slices[0].ImagePositionPatient  # 原点
    ...
    @ti.kernel
    def rpolling(valid_spheres: ti.types.ndarray()):
        """仅对有效的球心进行投票统计"""
    ...
    print(valid_spheres_np)
    if valid_spheres_np.shape[0] > 0:
        rpolling(valid_spheres_np)
        cirlocMax()
    ...
    for i in range(num_spheres):
        ...
        rpoll[tmp][4] = 0
    # xyz_pos.append(points[:, 0] * pixel_spacing[0] + ImagePositionPatient[0])
    lps_to_ras = np.array([[-1, 0, 0],
                        [0, -1, 0],
                        [0, 0, 1]])
    pos_ori = slices[-1].ImagePositionPatient
    print(111, pos_ori)
    print(pixel_spacing, thickness)
