...
def detect_spheres_from_dicom(data, pixel_spacing, thickness, pos_ori, output_points: int = 3, r_range: float = 2.5, rlen: int = 0,
    ...
    return np.array(output_points1)


if __name__ == '__main__':

    # dicom_path = "C:/Users/huang/Downloads/dicom_data_bad_01"
    dicom_path = os.path.join(os.path.expanduser("~"), "spine/dataStore/dicom_data")
    dicom_files = []
    ...
    spheres = detect_spheres_from_dicom(data=data, pixel_spacing=pixel_spacing, thickness=thickness, pos_ori=pos_ori, output_points=10, r_range=2.5)
    print(spheres)
